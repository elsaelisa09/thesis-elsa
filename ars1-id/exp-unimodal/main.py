...
class Config: 
    ...
    DATASET_DIR = os.path.join(os.path.dirname(os.path.dirname(CURRENT_DIR)), 'dataset')
    ...
    # Dataset mode: 'text_only' | 'image_only'
    DATASET_MODE = 'text_only'
...
def load_data(config):
    ...
    if config.DATASET_MODE == 'image_only':
    ...
    # Hanya load CLIPProcessor jika mode image_only
    clip_processor = CLIPProcessor.from_pretrained(config.CLIP_MODEL_NAME) if config.DATASET_MODE == 'image_only' else None
...
def main():
    ...
    if use_wandb:
        ...
        wandb_run_name = args.wandb_name if args.wandb_name else config.MODEL_NAME
    ...
    clip_model = CLIPModel.from_pretrained(config.CLIP_MODEL_NAME) if config.DATASET_MODE == 'image_only' else None
    electra_model = AutoModel.from_pretrained(config.ELECTRA_MODEL_NAME) if config.DATASET_MODE == 'text_only' else None

    print('Initializing fusion model...')
    if config.DATASET_MODE == 'text_only':
        model = TextOnlyElectra(
        ...
        model = ImageOnlyCLIP(
            clip_model=clip_model,
            electra_model=electra_model,
            fusion_img_dim=config.FUSION_TEXT_DIM,
            num_classes=config.NUM_CLASSES,
            freeze_encoders=True
        )
